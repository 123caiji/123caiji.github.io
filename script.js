...
document.addEventListener('DOMContentLoaded', function() {
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            setTimeout(() => {
                ...
                sendEmail({
                    ...
                    showNotification('发送失败，请稍后重试或直接联系我', 'error');
                }).finally(() => {
    ...
    function validateField(field) {
        ...
        // 长度验证
        else if (value && field.getAttribute('minlength') && value.length < parseInt(field.getAttribute('minlength'))) {
            errorMessage = `${fieldName}至少需要${field.getAttribute('minlength')}个字符`;
            isValid = false;
        }
    ...
    function addMouseFollower() {
        ...
        // 悬停时放大
        document.querySelectorAll('a, button, .card, .project-card, .blog-card').forEach(el => {
    ...
    function addTypewriterEffect() {
        ...
        function typeWriter() {
            if (i < text.length) {
                ...
                // 打字完成后闪烁光标
                setInterval(() => {
                    heroTitle.style.borderRight = heroTitle.style.borderRight === 'none' ? '2px solid var(--accent)' : 'none';
                }, 500);
            }
        }
    ...
    function add3DCardEffect() {
        const cards = document.querySelectorAll('.research-card, .publication-item, .certificate-item, .project-card, .blog-card');
